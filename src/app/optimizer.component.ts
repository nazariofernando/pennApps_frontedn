...
export class OptimizerComponent {
	...
	createGlider(): void {
		...
		this.alphaMorphService.createGlider(name, params, numberGen, "optimizer")

		this.af.database.object('/optimizer/' + name + '/0', { preserveSnapshot: true })
			.subscribe(snapshot => {
		...
		this.af.database.object('/optimizer/' + name + '/bestGlider').set(this.best)
